...
class GFSSegTrain(BaseDataset):
    ...
    def __init__(self, root, list_path, fold, shot=1, mode='train', crop_size=(512, 512),
        ...
        self.lbl_dir = 'annotations/train2014'
    ...
    def _get_train_sample(self, index):
        ...
        unlabel_id_list = self.filtered_base_unlabel_id_list
        id_b = random.choice(unlabel_id_list)
        ...
        label_b = self._convert_label(label_b, from_base=False)
        # date augmentation & preprocess
        ...
        image_s = self.cutout_context(image_s, label_b, n_holes=1, length=16)
        image_s = self.normalize(image_s)
        image_s = self.pad((224,224), image_s)
        image_s = self.totensor(image_s)
        image_w = image_b.copy()
        image_w = self.normalize(image_w)
        image_w = self.pad((224,224), image_w)
