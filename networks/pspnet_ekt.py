...
class GFSS_Model(nn.Module):
    ...
    def __init__(self, n_base, fold, shot=1, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        ...
        if is_ft:
            ...
            # # Initialize the parameters of novel_classifier with those from base_classifier
            # for base_layer, novel_layer in zip(self.classifier, self.classifier_n):
            #     if isinstance(base_layer, nn.Conv2d):
            #         novel_layer.weight.data.copy_(base_layer.weight.data)
            
            self.bias_linear = BiasLayer(n_novel)
    ...
    def orthogonal_decompose(self, feats, bases_b, bg=None):
        '''
            feats: [BxCxN]
            bases_b: [1xKxC]
            bases_n: [1xKxC]
        ...
        if bg is not None:
            s2 = F.normalize(bg.to(torch.float), p=2, dim=-1) # [1x1xC]
            bg_proj = torch.matmul(s2, q) # [Bx1xN]
            out_bg = bg_proj.unsqueeze(1) * s2.unsqueeze(-1) # [Bx1xCxN]
            return out_fg_b, out_bg
        else:
            out_bg = q - out_fg_b.sum(1) # [BxCxN]
            ...
            return out_fg_b, out_bg.unsqueeze(1), proj, units
    ...
    def forward(self, img, mask=None, img_w=None, img_s=None, s_img=None, s_mask=None):
        '''
            img       : [BxCxHxW]
            mask      : [BxHxW]
        '''
        if self.is_ft:
            if self.training:
                return self.forward_novel(img, mask, img_w, img_s)
            else:
                return self.forward_all(img, mask)
        else:
            return self.forward_base(img, mask, img_w, img_s)
    ...
    def forward_all(self, img, mask=None):
        ...
        n_class = 1 + base_emb.shape[0] + novel_emb.shape[0]
        
        '''cls_emb = cls_emb.expand(B, -1, -1)
        avg_pool = F.avg_pool2d(features, (h, w)) # [BxCx1x1]
        avg_pool = avg_pool.squeeze(-1).squeeze(-1) # [BxC]
        avg_pool = avg_pool.unsqueeze(1) # [Bx1xC]
        cls_emb = cls_emb + avg_pool # [Bx(Kbase+Knovel)xC]'''
        # # base to novel attention
        # K_b = self.k_linear_b(base_emb)
        # Q_n = self.q_linear_n(novel_emb)
        # V_b = self.v_linear_b(base_emb)
        # scores_b_n = torch.matmul(Q_n, K_b.transpose(0, 1))
        # attention_weights = F.softmax(scores_b_n, dim=-1)
        # attended_novel_emb = torch.matmul(attention_weights, V_b)
        ...
        cls_emb = torch.cat([base_emb, novel_emb], dim=1) # [1x(Kbase+Knovel)xC]
        out_fg, feats_bg, sim_maps, units = self.orthogonal_decompose(features.flatten(2), cls_emb) 
        sim_maps_reshape = sim_maps.view(B, n_class, h, w) # [Bx(Kbase+Knovel)xhxw]
        ...
        # outputs = torch.cat([out_fg_b, out_fg_n], dim=1) # [Bx(Kbase+Knovel)xCxN]
        # outputs = outputs.contiguous().view(B*(self.n_base+self.n_novel), C, h, w) # [(Bx(Kbase+Knovel))xCxhxw]
        ...
        # preds = preds.view(B, self.n_base+self.n_novel+1, h, w)
        return preds

    def forward_base(self, img, mask=None, img_w=None, img_s=None):
        '''
            img       : [BxCxHxW]
            mask      : [BxHxW]
        '''
        features = self.backbone.base_forward(img)
        features = self.decoder(features) # [BxCxhxw]
        B, C, h, w = features.shape # [BxCx473x473]
        cls_emb = self.base_emb.unsqueeze(0) # [1xKbasexC]

        n_class = 1 + cls_emb.shape[1] # base_emb.shape[1] + novel_emb.shape[1]
        out_fg, feats_bg, sim_maps, units = self.orthogonal_decompose(features.flatten(2), cls_emb)
        sim_maps_reshape = sim_maps.view(B, n_class, h, w) # [Bx(Kbase+Knovel+1)xhxw]
        feats_all = torch.cat([feats_bg, out_fg], dim=1) # [Bx(1+Kb)xCxN]
        feats_all = feats_all.reshape(B*n_class, C, h, w) # [(Bx(1+Kb))xCxhxw]
        preds = self.classifier(feats_all) # [(BxKb)x1xhxw]
        preds = preds.view(B, n_class, h, w) # [Bx(Kb+1)xhxw]
        ...
        if self.criterion is not None and mask is not None:
            # unlabeled_img = torch.cat([img_w, img_s], dim=0)
            # features_u = self.backbone.base_forward(unlabeled_img)
            # features_u = self.decoder(features_u)
            # B_u, C_u, h_u, w_u = features_u.shape # [BxKx224x224]
            # out_fg_u, feats_bg_u, _, _ = self.orthogonal_decompose(features_u.flatten(2), cls_emb)
            # feats_b_u = torch.cat([feats_bg_u, out_fg_u], dim=1) # [Bx(1+Kb)xCxN]
            # feats_b_u = feats_b_u.reshape(B_u*n_class, C_u, h_u, w_u) # [(Bx(1+Kb))xCxhxw]
            # preds_u = self.classifier(feats_b_u) # [(BxKb)x1xhxw]
            # preds_u = preds_u.view(B_u, n_class, h_u, w_u) # [BxKbxhxw]
            # preds_u_w = preds_u[:B_u//2] # [B_u//3:B//3*2] 
            # preds_u_s = preds_u[B_u//2:] # ][B//3*2:]
            ...
            # return self.criterion(preds.to(torch.float), mask, proto_sim=proto_sim, aux_preds=sim_maps_reshape), assist_loss
            return self.criterion(preds, mask, proto_sim=proto_sim, aux_preds=sim_maps_reshape)
        else:
            return preds

    def forward_novel(self, img, mask, img_w=None, img_s=None):
        '''
            img       : [BxCxHxW]
            mask      : [BxHxW]
        '''
        # with torch.no_grad():
        # img_full = torch.cat([img, img_w, img_s], dim=0)
        features_full = self.backbone.base_forward(img)
        ...
        # print('img_b:', img_b.shape, 'img_s:', img_s.shape)
        unlabeled_img = torch.cat([img_w, img_s], dim=0)
        features_u = self.backbone.base_forward(unlabeled_img)
        features_u = self.decoder(features_u) # [BxCxhxw]

        B, C, h, w = features.shape # [BxCx473x473]
        B_u, C_u, h_u, w_u = features_u.shape # [BxKx224x224]
        #base_emb = torch.stack([param for param in self.base_emb])
        #novel_emb = torch.stack([param for param in self.novel_emb])
        base_emb = self.base_emb # [1xKbasexC]
        novel_emb = self.novel_emb # [1xKnovelxC]
        # cls_emb = self.cls_emb.unsqueeze(0) # [1x1xC]
        
        '''cls_emb = cls_emb.expand(B, -1, -1)
        avg_pool = F.avg_pool2d(features_full, (h, w)) # [BxCx1x1]
        avg_pool = avg_pool.squeeze(-1).squeeze(-1) # [BxC]
        avg_pool = avg_pool.unsqueeze(1) # [Bx1xC]
        cls_emb = cls_emb + avg_pool # [Bx(Kbase+Knovel)xC]'''
        # #base to novel attention
        ...
        # attended_novel_emb = torch.matmul(attention_weights, V_b)
        # # #no parameters to learn, just use the cosine similarity
        # # scores_b_n = torch.matmul(novel_emb, base_emb.transpose(0, 1))  # 形状为 [5, 15]
        # # attention_weights = F.softmax(scores_b_n, dim=-1) 
        # # attended_novel_emb = torch.matmul(attention_weights, base_emb) # [1xKnovelxC]
        ...
        out_fg, feats_bg, sim_maps, units = self.orthogonal_decompose(features.flatten(2), cls_emb)
        out_fg_u, feats_bg_u, _, _ = self.orthogonal_decompose(features_u.flatten(2), cls_emb)
        out_fg_b_u, out_fg_n_u = out_fg_u[:, :self.n_base], out_fg_u[:, self.n_base:]
        feats_b_u = torch.cat([feats_bg_u, out_fg_b_u], dim=1) # [Bx(1+Kb)xCxN]
        feats_b_u = feats_b_u.reshape(B_u*(self.n_base+1), C_u, h_u, w_u) # [(Bx(1+Kb))xCxhxw]
        preds1_u = self.classifier(feats_b_u) # [(BxKb)x1xhxw]
        preds1_u = preds1_u.view(B_u, self.n_base+1, h_u, w_u) # [BxKbxhxw]
        feats_n_u = out_fg_n_u.reshape(B_u*(self.n_novel), C_u, h_u, w_u) # [(BxKn)xCxhxw]
        preds2_u = self.classifier_n(feats_n_u) # [(BxKn)x1xhxw]
        preds2_u = preds2_u.view(B_u, self.n_novel, h_u, w_u) # [BxKnxhxw]
        preds2_u = self.bias_linear(preds2_u)
        preds_u = torch.cat([preds1_u, preds2_u], dim=1) # [Bx(Kbase+Knovel+1)xhxw]
        ...
        # feats_n = torch.cat([feats_bg, out_fg_n], dim=1) # [Bx(1+Kn)xCxN]
        feats_n = out_fg_n.reshape(B*(self.n_novel), C, h, w) # [(BxKn)xCxhxw]
        preds2 = self.classifier_n(feats_n) # [(Bx(1+Kn))x1xhxw]
        preds2 = preds2.view(B, self.n_novel, h, w) # [BxKnxhxw]
        preds2 = self.bias_linear(preds2)
        ...
        # sim_maps_u_w = sim_maps[B//3:B//3*2]
        # sim_maps_u_s = sim_maps[B//3*2:]
        ...
        if self.criterion is not None and mask is not None:
            with torch.cuda.amp.autocast(enabled=False):
                ...
                return self.criterion(preds.to(torch.float), mask, is_ft=True, proto_sim=proto_sim), assist_loss
        else:
            return preds
