...
class GFSS_Model(nn.Module):
    ...
    def forward_novel(self, img, mask, img_w=None, img_s=None):
        ...
        # # ???one-hot??
        # preds_one_hot = F.one_hot(max_indices, num_classes=C)  # ??????[B, H, W, C]
        # # ??one_hot???????C,?????????????
        # preds_one_hot = preds_one_hot.permute(0, 3, 1, 2)  # ????,?????[B, C, H, W]
        # # one_hot???????,???????????soft_preds_u???????
