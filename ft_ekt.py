...
def get_parser():
    ...
    parser.add_argument("--resnet", type=str, default='/users/BAM-main/initmodel/backbones/resnet50_v2.pth')
    parser.add_argument("--cls-reload", action="store_true", default=False)
    return parser
...
def generate_protos(base_id_list, novel_id_list, args, norm_layer):
    ...
    # backbone = get_backbone(norm_layer=norm_layer, pretrained_model=args.resnet, backbone=args.backbone)
    ### base support samples
    base_images = []
    base_labels = []
    ...
    ### novel support samples
    novel_images = []
    novel_labels = []
    for i in range(len(novel_id_list)):
        id = novel_id_list[i]
        image = cv2.imread(osp.join(args.data_dir, img_dir, '%s.jpg'%id), cv2.IMREAD_COLOR) 
        label = cv2.imread(osp.join(args.data_dir, lbl_dir, '%s.png'%id), cv2.IMREAD_GRAYSCALE)
        image, label = dataset.resize(image, label, random_scale=True)
        ...
        label = label.unsqueeze(0)
        # print(image.shape, label.shape)
        novel_images.append(image)
        novel_labels.append(label)
    novel_images = torch.cat(novel_images, dim=0)
    novel_labels = torch.cat(novel_labels, dim=0)
    images = torch.cat([base_images, novel_images], dim=0) # [len(base_classes+novel_classes)*args.shot, 3, 473, 473]
    labels = torch.cat([base_labels, novel_labels], dim=0) # [len(base_classes+novel_classes)*args.shot, 473, 473]

    '''proto_avg = []
    for i in range(len(novel_classes)):
        per_novel_imgs = novel_images[i*args.shot:(i+1)*args.shot]
        per_novel_labels = novel_labels[i*args.shot:(i+1)*args.shot]
        protos = []
        for j in range(args.shot):
            img = per_novel_imgs[j]
            feature = backbone.base_forward(img)
            lbl = per_novel_labels[j]
            mask = (lbl == novel_classes[i]).float()
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for seed in seeds:
            ...
            trainset = eval('dataset.' + args.dataset + '_ft.GFSSegTrain')(
                ...
                mode='train', seed=seed, filter=args.filter_novel, use_base=True)
            ...
            testset = eval('dataset.' + args.dataset + '.GFSSegVal')(
                ...
                base_size=args.base_size, resize_label=True, use_novel=True, use_base=True)
            ...
            # support_imgs, support_labels = generate_protos(trainset.base_cls_to_ids, trainset.novel_id_list, args, BatchNorm)
            ...
            for epoch in range(args.start_epoch, args.num_epoch):
                ...
                # classifier_n = model.module.classifier_n
                # mean_n = classifier_n[4].weight.data.mean(dim=1, keepdim=True)
                # mean_b = classifier[4].weight.data.mean(dim=1, keepdim=True)
                # std_n = torch.std(classifier_n[4].weight.data, dim=1, keepdim=True)
                # std_b = torch.std(classifier[4].weight.data, dim=1, keepdim=True)
                # model.module.classifier_n[4].weight.data = classifier_n[4].weight.data - mean_n.mean() + mean_b.mean()
                # model.module.bias_linear.alpha.data = (std_b.mean() / std_n).reshape(model.module.bias_linear.alpha.data.shape)

                for i, data in enumerate(train_loader):
                    global_iteration += 1
                    img, mask, img_w, img_s = data
                    img, mask = img.cuda(non_blocking=True), mask.cuda(non_blocking=True)
                    img_w, img_s = img_w.cuda(non_blocking=True), img_s.cuda(non_blocking=True)
                    ...
                    with torch.cuda.amp.autocast(enabled=args.fp16):
                        loss_dict = model(img, mask, img_w, img_s)
                ...
                # torch.cuda.empty_cache()  # realising cache memory
