...
class Engine(object):
    def __init__(self, custom_parser=None):
        ...
        if self.distributed:
            ...
            torch.cuda.set_device(self.local_rank)  # set GPU for each process
    ...
    def data_parallel(self, model):
        if self.distributed:
            model = torch.nn.parallel.DistributedDataParallel(model.cuda(),device_ids=[self.local_rank], output_device=self.local_rank, find_unused_parameters=False)
        else:
